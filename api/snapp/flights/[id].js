...
module.exports = async function handler(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      const upstream = await fetch(`${base}/api/integration/flights/${encodeURIComponent(id)}`, {
        headers: { 'x-snapp-api-key': key },
      })
      const payload = await upstream.json().catch(() => ({}))
      if (!upstream.ok) {
        res.status(upstream.status).json({ error: payload.error || `SNAPP get failed (${upstream.status})` })
        return
      }
      res.status(200).json(mapFlight(payload.flight || payload))
      return
    }
    ...
    if (req.method === 'PATCH') {
      ...
      const payload = await upstream.json().catch(() => ({}))
      if (!upstream.ok) {
        res.status(upstream.status).json({ error: payload.error || `SNAPP update failed (${upstream.status})` })
        return
      }
      res.status(200).json(mapFlight(payload.flight || payload))
      return
